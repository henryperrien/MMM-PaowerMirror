...
module.exports = NodeHelper.create({
  launchit() {
    ...
    if (this.config.debug)
      console.log(
        "FaceRecognition spawning " +
          this.config.command +
          " using " +
          this.config.pythonName
      );
    handler = spawn(this.config.pythonName, ["-u", this.config.command]);
    handler.stdout.on("data", (data) => {
      if (this.config.debug)
        console.log("FaceRecognition sending program output=" + data);
      this.sendSocketNotification("message_from_helper", data.toString());
    });
    handler.stderr.on("data", (data) => {
      if (this.config.debug)
        console.log("FaceRecognition program error=" + data);
    });
    handler.on("error", (error) => {
      if (this.config.debug) console.log("FaceRecognition spawn error=" + data);
    });
  },
  startit() {
    if (this.config.command.startsWith(this.config.pythonName))
      this.config.command = this.config.command.slice(
        this.config.pythonName.length
      );
    if (this.config.localfolder)
      this.config.command = __dirname + path.sep + this.config.command;
    if (this.config.repetative) this.launchit();
    else {
      setInterval(() => {
        this.launchit();
